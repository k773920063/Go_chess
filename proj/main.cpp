...
int main()
{
	void Down_chess(int *turn,int Plat[7][7],int Plat_old[7][7],struct check_table *point,int XY[],float Grade[2]);
	void Draw_Plat_line();
	...
	float Grade[2]={0.0,0.0};	//Grade[0]为黑棋目数，Grade[1]为白棋目数，开局黑棋让一子 
	...
	while(1)
	{
		...
		Down_chess(&Turn,Plat,Plat_old,point,XY,Grade);
...
//后台操作类函数
void Down_chess(int *turn,int Plat[7][7],int Plat_old[7][7],struct check_table *point,int XY[],float Grade[2])		//落子函数
{
	int x=0,y=0,i=0,j=0;
	mouse_msg msg = {0};
	...
	if(x<=750)
	{
		...
		{
			//打劫违规
			MessageBox(NULL,"暂不能回提！","提示",MB_SYSTEMMODAL);
		}
		else
		{
			//已有落子
	...
	else if(x>=820&&x<=880&&y>=500&&y<=530)
	{
		if(Grade[0]>Grade[1])
		{
			MessageBox(NULL,"黑棋获胜","结果",MB_SYSTEMMODAL);
		}
		else if(Grade[0]<=Grade[1])
		{
			MessageBox(NULL,"白棋获胜","结果",MB_SYSTEMMODAL);
		}
		for(i=0;i<7;i++)
			for(j=0;j<7;j++)
			{
